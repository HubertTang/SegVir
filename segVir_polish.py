...
def segvir_parse_rst(ref_dir, temp_dir, blastp_evalue=1e-5, hmmer_evalue=1e-5, host_ratio_thres=0.84,
    ...
    contig_path = f"{temp_dir}/contig.fna"
    contig_seq_index = {}
    for s in SeqIO.parse(contig_path, 'fasta'):
...
def segvir_output(temp_dir, out_dir, outfmt=1):
    ...
    seq_region_dict = {}
    for contig_id, coor in zip(segvir_df['contig'], segvir_df['coordinate']):
    ...
    # for seq_id, coor_list in seq_region_dict.items():
    #     seq_region_dict[seq_id] = (min(coor_list), max(coor_list))

    seq_dict = {}
    for contig_id, family, coor, taxon, function in zip(segvir_df['contig'], segvir_df['family'], segvir_df['coordinate'], segvir_df['taxon'], segvir_df['function']):
